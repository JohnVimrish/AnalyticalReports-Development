...
def process_data(input_dict :dict, database_config :dict ) :
     ...
     def dataframe_csv_input(dft_column_list:list,csv_input_path:str):
     ...
     for input_key , input_value in input_dict.items() :
         ...
         dataframe_column_list,sql_column_list,csv_path   = input_value

         if isinstance(csv_path,list)  :
               execute_sql_commands (db_cursor,truncate_sql)
               for each_csv_input in input_value :
                         csv_input = dataframe_csv_input(dataframe_column_list,each_csv_input)
                         copy_activity(input_key, db_cursor ,database_object,sql_column_list,csv_input)
         else :
               execute_sql_commands (db_cursor,truncate_sql)
               csv_input = dataframe_csv_input(dataframe_column_list,csv_path)
               copy_activity(input_key, db_cursor ,database_object,sql_column_list,csv_input)